...
<div class="sb-auth-page">
    ...
    <div class="sb-auth-body" x-data="{ remember: false, showPwd: false }">
        ...
        <h1 class="sb-auth-title">{{ __('Prisijungti') }}</h1>
        ...
        <div class="sb-auth-remember">
            ...
            <label for="remember">{{ __('Prisiminti mane') }}</label>
        </div>

        <div class="sb-auth-divider">{{ __('arba prisijungti el. paštu') }}</div>
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div class="text-end mb-3">
                <a href="{{ route('password.request') }}" class="sb-auth-link">{{ __('Pamiršote slaptažodį?') }}</a>
            </div>

            <button type="submit" class="sb-auth-btn-primary">{{ __('Prisijungti') }}</button>
        </form>
